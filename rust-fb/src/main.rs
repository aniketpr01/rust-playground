...
async fn main() {
    let user = User{
        name: "Aniket Prajapati".to_string(),
        age: 35,
        email: "anikethprajapati@gmail.com".to_string()
    };
    let firebase = Firebase::new("https://rust-5a87f-default-rtdb.firebaseio.com/").unwrap();
...
async fn update_user(firebase_client: &Firebase, id: &String, user: &User) -> User {
    let firebase = firebase_client.at("users").at(&id);
    let user = firebase.update::<User>(&user).await;
